"""
ViewModel for ETF Watchlist page.

Manages multiple ETF watchlists with tabs.
"""
...
from src.domain.entities.contract import ETFContract
...
class ETFWatchlistViewModel(BaseViewModel):
    """
    ViewModel for ETF Watchlist page with multi-watchlist support.
    ...
    def __init__(
        ...
        log.info("ETFWatchlistViewModel initialized")
    ...
    def load_all_watchlists(self) -> None:
        ...
        try:
            # Get all watchlists
            watchlists = self._watchlist_service.get_all_watchlists("ETF")
            self.watchlists = watchlists

            # Get active watchlist ID
            active_id = self._watchlist_service.get_active_watchlist_id("ETF")

            # If no active, use first watchlist
            if not active_id and watchlists:
                active_id = watchlists[0]["id"]
                self._watchlist_service.set_active_watchlist_id("ETF", active_id)
    ...
    def switch_watchlist(self, watchlist_id: str) -> None:
        ...
        try:
            ...
            self._watchlist_service.set_active_watchlist_id("ETF", watchlist_id)
    ...
    def create_watchlist(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            new_watchlist = self._watchlist_service.create_watchlist("ETF", name)
    ...
    def delete_watchlist(self, watchlist_id: str) -> bool:
        ...
        try:
            ...
            result = self._watchlist_service.delete_watchlist("ETF", watchlist_id)
    ...
    def get_all_saved_assets(self) -> List[Asset]:
        """Get all saved ETF assets for selection dialog."""
        return self._watchlist_service.get_all_saved_assets("ETF")
    ...
    def asset_exists(self, symbol: str) -> bool:
        ...
        """
        return self._asset_service.exists("ETF", symbol)
    ...
    def fetch_and_create_asset(self, symbol: str) -> None:
        ...
        # ETFs use STK security type, same as stocks
        contract = ETFContract.create(
        ...
        self._asset_service.fetch_contract_details(
            "ETF", contract, callback=on_details_received
        )
    ...
    def _load_watchlist_symbols(self, watchlist_id: str) -> None:
        ...
        watchlist = self._watchlist_service.get_watchlist_by_id("ETF", watchlist_id)
    ...
    def _add_symbol(self, symbol: str = None) -> None:
        ...
        try:
            result = self._watchlist_service.add_symbol_to_watchlist(
                "ETF", self.active_watchlist_id, symbol
            )
    ...
    def add_symbols(self, symbols: List[str]) -> int:
        ...
        try:
            count = self._watchlist_service.add_symbols_to_watchlist(
                "ETF", self.active_watchlist_id, symbols
            )
    ...
    def select_symbol(self, symbol: str) -> None:
        ...
        if symbol:
            asset = self._asset_service.get_asset("ETF", symbol)
            self.selected_asset = asset

    def get_asset(self, symbol: str) -> Optional[Asset]:
        """Get asset for a symbol."""
        return self._asset_service.get_asset("ETF", symbol)
    ...
    def update_watchlist_order(self, symbols: List[str]) -> None:
        ...
        try:
            ...
            watchlist = self._watchlist_service.get_watchlist_by_id(
                "ETF", self.active_watchlist_id
            )
            if watchlist:
                # Use the service's update for the active watchlist
                self._watchlist_service.update_watchlist("ETF", symbols)
                self.symbols = symbols
    ...
    def _subscribe_realtime(self, symbol: str) -> None:
        ...
        req_id = self._realtime_service.subscribe("ETF", symbol, on_tick)
    ...
    def _unsubscribe_realtime(self, symbol: str) -> None:
        ...
        self._realtime_service.unsubscribe("ETF", symbol)
    ...
    def dispose(self) -> None:
        ...
        super().dispose()
        log.info("ETFWatchlistViewModel disposed")
